...
this.onmessage = function(e){
  ...
  switch(e.data.command){
    ...
    case 'startChunking' :
      ...
      transmitCommand("START", e.data.streamsIP); // IPAddress of Streams instance. 
      break;
    case 'stopChunking' :
 ...
 *   - store the id, cannot use the '#' remove it.
 * @param config
 */
...
function init(config) {
  ...
  wsSnd.onopen = function () {
    ...
    wsSnd.send("<CONTROL size=\"1024\">BINARY</CONTROL>");
...
/**
 * Convert the outbound audio data to ascii text, add a header
 * to beginning of each message. You need this on the relieving
 * side so it can keep track of he Audio source coming in.
 *
 *
 * @param audioBlob
 */
...
function transmitChunk(chunkBuffer) {
  ...
  var audioBlob = new Blob([dataview], { type: audioType });
